...
set which_xerces=xerces-1_4_3
set which_ibm=IBM_SQC_July10
set which_xsv=xsv12
...
:xerces
   ...
   if not exist %rootdir%xmlParsers\%which_xerces%\xerces.jar goto no-xer-jar
   ...
   set xerces_CLASSPATH=%CLASSPATH%;%rootdir%xmlParsers\%which_xerces%\xerces.jar
   set xerces_CLASSPATH=%xerces_CLASSPATH%;%rootdir%xmlParsers\%which_xerces%\xercesSamples.jar

   java -classpath %xerces_CLASSPATH% sax.SAXCount -v %2
...
:msxml
   if not exist ..\xsdms.js goto no-xsdms
   if not exist %2 goto no-file
   if not exist %3 goto no-xsd

   echo.
   echo ---------------------------------------------------------
   echo Running the MSXML4.0 Schema Validator with the following specification:
   echo    - XML File Being Validated: %2
   echo    - XSD File: %3
   echo ---------------------------------------------------------
...
:no-xer-jar
   cls
   echo Oops! Missing this xerces file: %rootdir%xmlParsers\%which_xerces%\xerces.jar
...
:no-xsdms
   cls
   echo Oops! Missing this javascript file: ..\xsdms.js
